...
export interface ServiceHints {
  ...
  /**
   * 사람이 확인해서 **웹에는 가입 창구가 아예 없다**는 것을 알았을 때 true.
   *
   * 토스·카카오톡·티맵·타다 같은 앱 우선 서비스는 웹사이트가 소개 페이지다.
   * 가입은 앱 안에서만 이루어진다. 그런 곳을 "아직 확인 못 함"으로 두면 게을러서
   * 안 잰 것처럼 읽히는데, 사실은 **웹에서 잴 수 있는 것이 없다**는 뜻이고
   * 그건 외국인이 알아야 할 정보다 — "이건 앱을 깔아야 한다"는 뜻이니까.
   *
   * 값을 대신하지는 않는다. signup_phone_auth 는 여전히 비어 있고, 화면에서
   * 비어 있는 이유만 "가입은 앱에서만 됩니다"로 바뀐다.
   */
  signup_app_only?: boolean | null;
}
